...
class RepVGGStage(nn.Module):
    def __init__(self, in_ch, stage_ch, num_block, kernel_size=3, groups=1,
            ...
            if num == 0:
                LayerDict["Block{}".format(num)] = \
                    ConvModule(in_ch, stage_ch, kernel_size,
                               ...
                               conv_cfg=conv_cfg, act_cfg=act_cfg, norm_cfg=norm_cfg)
                continue
            LayerDict["Block{}".format(num)] = \
                ConvModule(stage_ch, stage_ch, kernel_size, groups=groups, stride=1, padding=padding,
                           conv_cfg=conv_cfg, act_cfg=act_cfg, norm_cfg=norm_cfg)
...
class RepVGGNet(nn.Module):
    def __init__(self,
        ...
        for num_stages in range(self.stage_nums):
            stage = RepVGGStage(in_channel, stage_channels[num_stages], block_per_stage[num_stages],
                                kernel_size=kernel_sizes[num_stages], groups=1, conv_cfg=conv_cfg, act_cfg=act_cfg)
